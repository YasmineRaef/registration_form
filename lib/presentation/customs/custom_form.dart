...
class Validator {
  ...
  static String? checker(String hintText, String value) {
    switch (hintText) {
      ...
      case 'Username':
        user = value;
      //after validating you want to check if the username is already taken or not ..
      case 'Password':
        correctPass = value;
        if (value.length < 6) {
    ...
    if (email == '' && user != '') {
      //case sign-up i.e-->username has value
      storeUsers(email, correctPass);
    } else if (email != '' && user == '') {
      //case sign-in i.e-->email has value
      checkInputs(email, correctPass);
    }
